...
export const EXTENSION_CONFIG_KEY = "SAS.session";
 ...
 */
export async function addValueToGlobalConfig(
  configKey: string,
  value: string
): Promise<void> {
  await addValueToConfigAtScope(
    configKey,
    value,
...
export function getConfigFile(): string {
  let configPath =
    vscode.workspace.getConfiguration(EXTENSION_CONFIG_KEY)["configFile"];
  if (!configPath) {
