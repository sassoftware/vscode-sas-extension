...
export class CompletionProvider {
  ...
  private _getSemicolonTriggeredIndentEdit(
    ...
    const actualCurLineIndent = this._getIndentLength(curLineText, tabSize);

    if (expectedCurLineIndent === actualCurLineIndent) {
      return [];
    } else {
  ...
  private _getIndentIncrementOfNextLine(
    ...
    do {
      ...
      const semicolonToken: SyntaxToken = cleanedTokens[curIndex];
      const prevToken = cleanedTokens[curIndex - 1]; // curIndex must be > 0
      const prevTokenText = this._getTokenText(
        cleanedTokens,
        curIndex - 1,
        lineText,
      )?.trim();
  ...
  private _isCommentOrBlankToken(token: SyntaxToken, text: string): boolean {
    ...
    if (token.style === TOKEN_TYPES.COMMENT) {
      return true;
    }
