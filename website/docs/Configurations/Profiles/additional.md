...
Autoexec code can be configured for SAS Viya and ITC-based connection profiles ("remote - IOM" and "local" connection types). The configured autoexec code executes each time you start a new SAS session. Changes to the autoexec code do not take effect until you close and restart your SAS session.

The Autoexec option supports different modes for how to define the SAS lines that should run:
      ...
      "profiles": {
        "sas9IOM": {
          ...
          "autoExec": [
            {
              "type": "file",
              "filePath": "C:\\path\\to\\autoexec.sas"
            }
          ]
        }
      }
